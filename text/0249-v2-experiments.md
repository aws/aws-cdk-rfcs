...
in the `.d.ts` files. Users will be able to cast this away using `as any` though.
They will also be able to confuse the compiler enough that they will get to pass
struct properties that are technically `@experimental`.
...
code (the `package.json` would contain: `{ dependencies: {
...
`aws-cdk-lib-experimental` under the name `aws-cdk-lib`, and no `require()` statements
would have to be changed.

An application trying to use experimental will do this:

```json
{
  "dependencies": {
      "aws-cdk-lib": "npm:aws-cdk-lib-experiments@1.60.0",
      "some-construct-lib-that-uses-stable": "^1.2.3"
  }
}
```
...
but we lose the tool-checked dependency on experimental; from NPM's
point of view the library depends on stable CDK, and it's only
notes in the README that can inform the consumer otherwise.
