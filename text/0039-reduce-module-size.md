...
2. Modify the `lambda-layer-X` submodules in `aws-cdk-lib` to not bundle these
   ...
   implemented once, and then autogenerated or reused in each `aws-cdk-lib/lambda-layer-X`
   ...
   reference them.
    1. I have implemented a POC that shows the proposed dyanmic loading mechanism will
       work with JSII target languages. See [this repo]() for more details.
       ...
       to this package.
        1. [kubectl](https://kubernetes.io/docs/tasks/tools/#kubectl)
            ...
            4. A weekly automated task will check for new supported minor
               ...
               be created which creates a new `Layer` class in the API for the
               new minor version. A human, the on-call engineer, will need to
               review this one.
...
We have two options to make them smaller:
1. Compress the .jsii.tabl.json and .jsii files. Compressing with gzip
