...
- **Stage 0 activities:**
  - **Automatically publish package** - a package for the module is autogenerated and published in the aws-cdk GitHub repo
  ...
  - **Create GitHub tracking issue** - create a GitHub tracking issue for the module.
  The primary role of the tracking
    issue is to:
    ...
    > **Automation Opportunity:** When a new module is automatically added to the AWS Construct Library, also auto-generate the module's tracking issue so that it can be assigned an owner as part of the Open Source Developer Support (OSDS) team's issue triage process.
...
- **Stage 2 activities**:
  ...
  - **Observe** - monitor GitHub issues filed against the developer preview module and module utilization metrics
    - feature requests and bugs filed against the developer preview module are triaged, and those that we decide to include
      in the module are linked in the tracking issue to indicate they are in scope. All P0 issues are fixed immediately,
      P1 issues are in scope for GA release, and P2 issues are deferred until we accumulate enough feedback to change
      its priority.
    - report the number of stacks that include -experimental packages on a weekly basis to measure customer interest and
