...
- **Stage 0 activities:**
  - **Publish package** - a package for the module is autogenerated and published in the aws-cdk GitHub repo
    ...
    - in the module's `package.json` set "Stability": "Stable" to indicate the CloudFormation resource constructs are stable and supported by AWS  
  ...
  - **Update Roadmap** - add the module's tracking issue to the public roadmap in the "wish list" column so that it's easy for customers to discover and +1. 
...
- **Stage 1 activities:**
  ...
  - **Identify key stakeholders** - Opportunistically recruit stakeholders outside the CDK team to participate in API reviews and usability studies. Potential stakeholders include: 
    ...
    - solutions architects who specialize in service (sourced via [Technical Field Communities](https://w.amazon.com/bin/view/AWS/Teams/TFC)), 
  ...
  - **Publish [service_name]-experimental package** - deliver experimental constructs in their own package to clearly
    ...
    - in the module's `package.json` set "Stability": "Experimental" to indicate the high-level constructs are under active development and subject to breaking changes
...
- _"Favor community contributions above all"_ - customers who take the time to contribute code to the AWS CDK get preferential treatment above all else
