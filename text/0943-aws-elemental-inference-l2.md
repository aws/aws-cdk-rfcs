...
`Feed` and `Dictionary` are stateless configuration — no data at rest, no accumulated state. The CDK Design
Guidelines reserve `removalPolicy` for stateful resources where deletion means data loss. Neither applies here.
